...
TODO: actually compute this here
```{r}
homm_data = data.frame(experiment_phase = c("testing_basic", "testing_losing"),
                       normalized_expected_earnings = c(0.503906, 0.250000))

```

```{r}
homm_data = homm_data %>%
  mutate(normalized_expected_earnings = normalized_expected_earnings / 2) # normalize by max bet
```
...
#  geom_hline(yintercept=-1, linetype=2, alpha=0.5) +
  ...
  labs(x="Experiment phase", y="Normalized earnings") +
...
ggplot(cards_data %>% 
  ...
  annotate("text", x=0.57, y=0.55, alpha=0.5, label="Chance") +
  annotate("text", x=0.57, y=0.95, alpha=0.5, label="Optimal") +
  guides(color=F)
